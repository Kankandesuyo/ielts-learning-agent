...
- 增加密码重置、管理员角色、周/月学习统计仪表盘和提醒功能。
- SQLite 切换 PostgreSQL，支持多用户长期使用。
- 增加安全措施：限流、日志脱敏、API Key 托管和生产密钥轮换。

## 安全措施

- 邮箱注册和登录使用 scrypt 加盐密码哈希，不保存明文密码。
- 登录采用服务端可撤销会话；原始会话令牌只保存在 HttpOnly Cookie，数据库只保存令牌摘要。
- 修改数据的请求同时校验 SameSite Cookie 和 CSRF 双令牌。
- 服务端将账号绑定到唯一学习画像，拒绝读取或修改其他 `user_id` 的画像、计划、错题和资料。
...
- 不声称官方评分，避免误导学生。
- 公网部署时必须启用 HTTPS 和 `SECURE_COOKIES=true`，并继续加入请求限流、日志脱敏和数据库迁移。

## 12. 2026-07-20 可运行状态

- 已建立 Git 仓库、安全忽略规则和首个可回退提交 `512822e`。
- 已实现邮箱注册、登录、退出、会话恢复、CSRF 防护和账号级数据所有权隔离。
- 已实现画像读取、修改、删除。
- 学习计划会持久化到 `study_plans` 和 `study_plan_days`，支持每日完成/撤销打卡和进度百分比。
- 已把本地资料索引、来源页码出题和答案分析纳入前端与 API。
- 当前自动化测试结果：`33 passed`，后端行覆盖率 `89%`；新增认证逻辑后总代码量上升，测试同时调用前端 XSS 安全脚本。
- 已真实启动服务验证首页、画像创建、两天计划生成、Day 1 打卡、最近计划读取，进度正确返回 `50%`。
- Playwright 真实浏览器已验证注册、画像创建、刷新保持登录、退出阻断访问，以及桌面和 390×844 手机布局；控制台 0 错误、0 警告。
- 当前已具备基础多账号隔离，但公开部署前仍需 HTTPS、安全 Cookie、限流、密码重置、日志脱敏和数据库迁移。
